...
class PrivacyPolicyViewController: UIViewController, MFMailComposeViewControllerDelegate {
    ...
    if sender.didTapAttributedTextInLabel(label: self.detailedLabel, inRange:NSRange(location: 1259, length: 20)){
       let webView = WebViewViewController()
        webView.setLinkURL(link: Constant.URLConstants.googlePlay)
       self.navigationController?.pushViewController(webView, animated: true)

    }else if sender.didTapAttributedTextInLabel(label: self.detailedLabel, inRange: NSRange(location: 4493, length: 21)){
        self.sendEmail()
        }
    }
