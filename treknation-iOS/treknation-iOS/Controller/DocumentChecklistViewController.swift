...
class DocumentChecklistViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        attributedString.addAttribute(.foregroundColor, value: UIColor(red: 2.0 / 255.0, green: 167.0 / 255.0, blue: 220.0 / 255.0, alpha: 1.0), range: NSRange(location: 729, length: 21))
    ...
    @objc func handleTap(_ sender: UITapGestureRecognizer) {
                 ...
                 if sender.didTapAttributedTextInLabel(label: self.detailedLabel, inRange:NSRange(location: 837, length: 14)){
                    ...
                    self.navigationController?.pushViewController(webView, animated: true)
                     ...
                     self.navigationController?.pushViewController(webView, animated: true)
                 }else if sender.didTapAttributedTextInLabel(label: self.detailedLabel, inRange: NSRange(location: 729, length: 46)){
                     let webView = WebViewViewController()
                      webView.setLinkURL(link: Constant.URLConstants.digitalPhotoLink)
                     self.navigationController?.pushViewController(webView, animated: true)
                 } else {
                     print("Tapped none")
                 }
